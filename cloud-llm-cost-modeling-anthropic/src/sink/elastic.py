"""Bulk indexer for Elastic data streams with batching + retry."""
import json
import time
...
class BulkSink:
    def __init__(self, batch_docs: int = 2000):
    ...
    def flush(self):
        ...
        for (idx, _), item in zip(self.buf, resp["items"]):
            ...
            if res.get("status", 500) < 300:
                ...
                self.failed[idx] += 1
                self.error_samples.setdefault(
                    idx, json.dumps(res.get("error"))[:400])
        self.buf = []
...
def es_search(index: str, body: dict) -> dict:
    r = requests.post(f"{ELASTIC_URL}/{index}/_search", headers=ES_HEADERS,
